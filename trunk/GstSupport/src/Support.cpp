...
{	
    ...
    namespace Pipeline
    {
        ...
        GstState GetState(const GstElement * inElement)
        {
            ...
            GstStateChangeReturn ret = gst_element_get_state(const_cast<GstElement*>(inElement), &state, &pending, 1000*1000*1000);
    ...
    namespace Bin
    {
        ...
        GstElement * Create(const std::string & inName)
        {
            ScopedObject<GstElement> result(gst_bin_new(inName.c_str()));
            if (!result)
            {
                throw std::runtime_error("Failed to create GstBin object.");
            }            
        ...
        {
            ScopedObject<GstElement> result(Create(inName));
            if (!gst_bin_add(GST_BIN(inPipeline), result.get()))
            {
                throw std::runtime_error("Failed to add bin to pipeline.");
            }            
            return result.release();
        }
        ...
        {        
            GstElement * result = 0;
            GError * error(0);
            result = gst_parse_bin_from_description(inDescription.c_str(), true, &error);
            if (error)
            {
                std::string msg = std::string("Parsing bin description failed. Reason: ") + error->message;
                throw std::runtime_error(msg);
            }
            return result;
        }
        ...
        {
            if (!gst_bin_add(GST_BIN(inBin), inElement))
            {
        ...
        {
            if (!gst_bin_remove(GST_BIN(inBin), inElement))
            {
                throw std::runtime_error("Failed to remove element to bin");
            }
        }
        
        GstElement * GetElementByName(GstBin * inBin, const std::string & inName)
        {
            ScopedObject<GstElement> result(gst_bin_get_by_name(inBin, inName.c_str()));
            if (!result)
            {                
    ...
    namespace Element
    {
        ...
        {
            GstElement * element = gst_element_factory_make(inType.c_str(), inName.c_str());
            if (!element)
            {
        ...
        {
			ScopedObject<GstElement> element(Create(inType, inName.c_str()));
			if (!gst_bin_add(GST_BIN(inPipeline), element.get()))
            {
        ...
        void SetState(GstElement * inElement, GstState inState)
        {
            ...
            if (GST_STATE_CHANGE_FAILURE == ret)
            {
                std::string msg = Gst::MakeString() << "Failed to change the element state to: " << Gst::Logging::ToString(inState) << ". ";
                throw std::runtime_error(msg);
            }
        }
        ...
        {    
            ScopedObject<GstPad> pad(gst_element_get_static_pad(inElement, inName.c_str()));
            if (!pad)
            {
                std::string msg(Gst::MakeString() << "Failed to get static pad with name " << inName);
                throw std::runtime_error(msg);
            }
            return pad.release();
        }
        ...
        {    
            ScopedObject<GstPad> pad(gst_element_get_request_pad(inElement, inName.c_str()));
            if (!pad)
            {
    ...
    namespace Logging
    {
        ...
        std::string ToString(GstStateChangeReturn inStateChangeReturn)
        {
            switch (inStateChangeReturn)
            {
                ...
                default:
                {
                    throw std::runtime_error("Invalid GstStateReturn.");
                }
            }
        }
        ...
        std::string ToString(GstMessageType inMessageType)
        {
            switch (inMessageType)
            {
                ...
                default:
                {
                    throw std::logic_error("Unknown GstMessageType");
                }
            }
        }
